...
def temp_tuning(image: np.array, simulated_image: np.array,  T_MAX):
    ...
    t_step = np.exp((np.log(t_min) - np.log(T_MAX)) / tuning_iterations)
    #t_step = (T_MAX - t_min) / tuning_iterations
    T = T_MAX
    ...
    p_min = 0.02
    p_max = 0.98
    t_min_c = fit_params[1] - (fit_params[3]/fit_params[0])*np.log((1.0/p_min)-1.0)
    t_max_c = fit_params[1] - (fit_params[3]/fit_params[0])*np.log((1.0/p_max)-1.0)
    #print(temp_tstar, np.exp(temp_tstar))
    #print(t_min_c, t_max_c)

    #t_min = max(np.log(t_min), temp_tstar-2.0)
    #t_max = min(np.log(T_MAX), temp_tstar+2.0)
    t_min = t_min_c
    t_max = t_max_c
    print(np.exp(t_min), np.exp(t_max))

    plt.figure(4)
    plt.plot((temperature), accept_rate, 'bo')
    plt.plot((temperature), sigmoid(np.log(temperature), *fit_params), 'r-')
    plt.ylabel('Acceptance Rate')
    plt.xlabel('Temperature')
    plt.xscale('log')
    plt.show()
   ...
   ############## TEMP RANGE 1 ###################
    ...
    # plt.figure(4)
    # plt.plot((temperature), accept_rate, 'ro')
    # plt.plot((temperature), sigmoid(np.log(temperature), *fit_params), 'b-')
    # plt.ylabel('Acceptance Rate')
    # plt.xlabel('Temperature')
    # plt.xscale('log')
    # plt.show()
